 ...
 * (measured as the number of days Math.since 1/1/4713 BC noon UCT, e.g., 2451545.125 is 1/1/2000 15:00 UTC).
 ...
 * Parameter: floating number jdn, the number of days Math.since 1/1/4713 BC noon
 * Algorithm from: "Astronomical Algorithms" by Jean Meeus, 1998
 */
...
export function getCanChiMonth(month, year) {
...
export function getCanHour(jdn) {
  return CAN[(jdn - 1) * 2 % 10];
}

export function getCanDay(jdn) {
  var dayName;
...
export function getBeginHour(jdn) {
  return CAN[(jdn - 1) * 2 % 10] + ' ' +  CHI[0];
}

export function getCanChiHour(gio, type = "lunar") {
  const canHour = (gio + 1) % 24;
  const typeHour = type === "lunar" ? 1 : 0;
  const indexCan = (canHour - 1) % 10;
  const indexChi = (typeHour * 12 + canHour - 1) % 12;
